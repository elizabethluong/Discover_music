...
def callback():
    ...
    # Check existing user.
    if id and email:
        existing_user = User.query.filter(User.id == id or User.email == email).first()
        if existing_user:
...
def get_playlists(token, id):
    ...
    playlist_api_endpoint = "https://api.spotify.com/v1/users/{}/playlists".format(user_id)
...
def get_genres(token):
    ...
    genre_endpoint_response = requests.get(genre_endpoint, headers=authorization_header)
...
def get_tracks(token, genre):
    ...
    authorization_header = {"Authorization": "Bearer {}".format(access_token)}      
    # Get genre data
    tracks_endpoint = "https://api.spotify.com/v1/recommendations?seed_genres=" + genre
    tracks_endpoint_response = requests.get(tracks_endpoint, headers=authorization_header)
...
def follow_artist(token, artist_id, user_id):
    ...
    follow_artist = Followed(body=artist_id,timestamp=dt.now(),user_id=user_id)
    ...
    follow_endpoint_response = requests.put(follow_endpoint, headers=authorization_header)
